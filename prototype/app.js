const videos = [
  { title: "Gardens of the Reef", category: "Coral reefs", duration: "04:28", description: "A slow journey through vibrant coral structures and their resident fish.", tags: ["coral", "reef", "fish"], streamKey: "clownFish", image: "https://images.unsplash.com/photo-1546026423-cc4642628d2b?auto=format&fit=crop&w=1200&q=80" },
  ...
  { title: "A School in Motion", category: "Fish", duration: "03:55", description: "Observe how a large school of fish moves as one coordinated group.", tags: ["fish", "school", "movement"], streamKey: "purpleFish", image: "https://images.unsplash.com/photo-1544550285-f813152fb2fd?auto=format&fit=crop&w=1200&q=80" },
  { title: "Reef at Night", category: "Coral reefs", duration: "07:06", description: "A night-time reef reveals creatures and patterns hidden during daylight.", tags: ["coral", "night", "reef"], streamKey: "redFish", image: "https://images.unsplash.com/photo-1518467166778-b88f373ffec7?auto=format&fit=crop&w=1200&q=80" },
...
function renderVideos() {
  ...
  grid.innerHTML = shown.map((video, index) => `<article class="video-card"><button type="button" class="video-open" data-index="${videos.indexOf(video)}"><img src="${video.image}" alt="${video.title}" loading="lazy" /><span class="play" aria-hidden="true">▶</span><span class="duration">${video.duration}</span></button><div class="card-copy"><p>${video.category}</p><h3>${video.title}</h3><button type="button" class="text-link" data-index="${videos.indexOf(video)}">View film <span aria-hidden="true">↗</span></button></div></article>`).join("");
...
document.querySelectorAll(".filter").forEach((button) => button.addEventListener("click", () => { category = button.dataset.category; document.querySelectorAll(".filter").forEach((filter) => filter.classList.toggle("active", filter === button)); renderVideos(); }));
...
function closeDialog() { player.pause(); if (hls) { hls.destroy(); hls = undefined; } player.removeAttribute("src"); player.load(); dialog.close(); }
